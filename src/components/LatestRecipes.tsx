...
function LatestRecipes() {
    ...
    async function getRecipes() {
        ...
        const response = await fetchRecipes({random: true, mealType: type})
            .then(response => response.hits.slice(0, 4))

        for (let i = 0; i < 4; i++) {
            response[i].recipe['id'] = v1()
        }

        setLatestRecipes(response)
    }

    useEffect(() => {
        if (latestRecipes.length === 0) getRecipes()
    }, [])
                        ...
                        <div key={el.recipe.id} className='latest-recipe__container'>
                            <div className='latest-recipe'>
                                ...
                                <Link className='link' to={'/recipes/1'}>
                                    <div className='lr-title'><span>View recipe</span></div>
                                </Link>
                                <div className='description'>
                                    <span>{el.recipe.label}</span>
                                </div>
                            </div>
                        </div>
                    )
