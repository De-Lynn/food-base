import '../styles/RandomRecipe.scss'
...
function RandomRecipe() {
    ...
    async function getRecipe() {
        ...
        const response = await fetchRecipes({random: true, cuisineType: type})
            .then(response => response.hits[0])

        response.recipe['id'] = v1()
        setRandomRecipe(response)
    }

    useEffect(() => {
        if (randomRecipe === null) getRecipe()
    }, [])
    ...
    else return (<div className='random-recipe'></div>)
}
